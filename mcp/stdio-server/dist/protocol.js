...
const tools = [
    ...
    },
    {
        ...
        handler: async (args, id) => {
            ...
            if (isNaN(listingId))
                return buildError(id, -32602, 'Invalid id');
    ...
    },
    {
        ...
        handler: async (args, id) => {
            ...
            if (updates.length === 0)
                return buildToolResult(id, JSON.stringify({ listing: current.rows[0], meta: { warnings: ['No fields provided'] } }, null, 2));
    ...
    },
    {
        ...
        handler: async (args, id) => {
            const data = await query('INSERT INTO enquiries (name, email, phone, message, property_id) VALUES ($1,$2,$3,$4,$5) RETURNING *', [String(args.name), String(args.email), args.phone ?? null, String(args.message), args.property_id ?? null]);
            return buildToolResult(id, JSON.stringify(data.rows[0], null, 2));
        },
    },
];
function buildToolResult(id, text, isError = false) {
    return { jsonrpc: '2.0', id, result: { content: [{ type: 'text', text }], isError } };
}
function buildError(id, code, message) {
    return { jsonrpc: '2.0', id, error: { code, message } };
}
export function buildInitializeResponse(id) {
    return {
        jsonrpc: '2.0', id,
        result: {
...
export function buildToolsListResponse(id) {
...
export async function handleToolCall(request) {
    ...
    if (!id)
        return buildError(0, -32600, 'tool call requires an id');
...
export async function handleJsonRpcMessage(request) {
